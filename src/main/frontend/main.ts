...
window.addEventListener("DOMContentLoaded", () => {
  ...
  if (path === "/" || path.endsWith("/home.html")) {
    ...
    try {
      ...
      const recents: { id: string; name: string; coverUrl: string | null }[] = raw ? JSON.parse(raw) : [];
    ...
    const btn = document.getElementById("use-link") as HTMLButtonElement | null;
    const ta = document.getElementById("playlist-url") as HTMLTextAreaElement | null;
    const gen = document.getElementById("generated-links");
    const apiA = document.getElementById("api-link") as HTMLAnchorElement | null;
    const pageA = document.getElementById("playlist-page-link") as HTMLAnchorElement | null;
    ...
    if (btn && ta) {
      btn.addEventListener("click", async () => {
        ...
        try {
          ...
          if (!parsed.hostname.includes("spotify.com")) throw new Error("Not a Spotify URL");
          const parts = parsed.pathname.split("/");
          if (parts[1] !== "playlist" || parts.length < 3) throw new Error("Not a playlist URL");
          id = parts[2].split("?")[0];
        ...
        try {
          ...
          if (existingIndex >= 0) {
            ...
            if (recents.length > 10) recents.length = 10; // Limit to 10 recent playlists
          }
