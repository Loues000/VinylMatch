export async function injectHeader() {
    ...
    try {
        ...
        const createSpotifyButton = (loggedIn: boolean) => {
        ...
        const updateSpotifyButton = (loggedIn: boolean) => {
            const navRight = container.querySelector(".navigation.navigation-right") as HTMLElement;
            if (!navRight) return;

            // Remove old
            const oldBtnLi = navRight.querySelector("#spotify-login-btn")?.parentElement;
            if (oldBtnLi) oldBtnLi.remove();

            // Add new
            navRight.appendChild(createSpotifyButton(loggedIn));

            // Event hinzufügen
            const spotifyBtn = container.querySelector<HTMLAnchorElement>("#spotify-login-btn");
            if (spotifyBtn) {
